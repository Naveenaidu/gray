...
func TestAddTuples(t *testing.T) {
	...
	a1 := geometry.NewPoint(3, -2, 5)
	a2 := geometry.NewVector(-2, 3, 1)
	a3 := a1.AddVector(*a2)

	if !a3.IsEqual(*geometry.NewPoint(1, 1, 6)) {
		t.Errorf("got: %+v, want: (1, 1, 6, 1)", a3)
	}

	// Case 2: Adding two vectors gives a new vector
	c2v1 := geometry.NewVector(3, -2, 5)
	c2v2 := geometry.NewVector(-2, 3, 1)
	c2v3 := geometry.AddVectors([]geometry.Vector{*c2v1, *c2v2})
	if !c2v3.IsEqual(*geometry.NewVector(1, 1, 6)) {
...
func TestSubtractTuples(t *testing.T) {
	...
	c1p1 := geometry.NewPoint(3, 2, 1)
	c1p2 := geometry.NewPoint(5, 6, 7)
	c1v1 := c1p1.Subtract(*c1p2)

	if !c1v1.IsEqual(*geometry.NewVector(-2, -4, -6)) {
	...
	c2p1 := geometry.NewPoint(3, 2, 1)
	c2v1 := geometry.NewVector(5, 6, 7)
	c2p2 := c2p1.SubtractVector(*c2v1)
	if !c2p2.IsEqual(*geometry.NewPoint(-2, -4, -6)) {
		t.Errorf("got: %+v, want: (-2, -4, -6, 1)", c2p2)
	}

	// Case 3: Subtract two vectors, gives a new vector
	c3v1 := geometry.NewVector(3, 2, 1)
	c3v2 := geometry.NewVector(5, 6, 7)
	c3v3 := geometry.SubtractVectors([]geometry.Vector{*c3v1, *c3v2})
	if !c3v3.IsEqual(*geometry.NewVector(-2, -4, -6)) {
		t.Errorf("got: %+v, want: (-2, -4, -6, 0)", c3v3)
	}

	// Case 4: Subtract a vector from itself, gives a zero vector
	c4v1 := geometry.NewVector(3, 2, 1)
	c4v2 := geometry.SubtractVectors([]geometry.Vector{*c4v1, *c4v1})
	if !c4v2.IsEqual(*geometry.NewVector(0, 0, 0)) {
...
func TestNegateVector(t *testing.T) {
	a1 := geometry.NewVector(1, -2, 3)

	if !a1.Negate().IsEqual(*geometry.NewVector(-1, 2, -3)) {
		t.Errorf("got: %+v, want: (-1, 2, -3, 0)", a1.Negate())
	}
}

func TestScalarMuliply(t *testing.T) {
	p1 := geometry.NewPoint(1, -2, 3)
	p2 := p1.ScalarMultiply(3)
	if !p2.IsEqual(*geometry.NewPoint(3, -6, 9)) {
		t.Errorf("got: %+v, want: (3, -6, 9, 1)", p2)
	}
	v1 := geometry.NewVector(1, -2, 3)
	v2 := v1.ScalarMultiply(3)
	if !v2.IsEqual(*geometry.NewVector(3, -6, 9)) {
...
func TestScalarDivide(t *testing.T) {
	...
	if !p2.IsEqual(*geometry.NewPoint(0.5, -1, 1.5)) {
		t.Errorf("got: %+v, want: (0.5, -1, 1.5, 1)", p2)
	}
	v1 := geometry.NewVector(1, -2, 3)
	v2 := v1.ScalarDivide(2)
	if !v2.IsEqual(*geometry.NewVector(0.5, -1, 1.5)) {
...
func TestNormalizeVector(t *testing.T) {
	var v, v_normalized *geometry.Vector

	v = geometry.NewVector(4, 0, 0)
...
func TestDotProduct(t *testing.T) {
	a := geometry.NewVector(1, 2, 3)
	b := geometry.NewVector(2, 3, 4)
...
func TestCrossProduct(t *testing.T) {
	...
	if !abCrossProduct.IsEqual(*geometry.NewVector(-1, 2, -1)) {
		t.Errorf("got: %+v, want: (-1, 2, -1)", abCrossProduct)
	}

	baCrossProduct := b.CrossProduct(*a)

	if !baCrossProduct.IsEqual(*geometry.NewVector(1, -2, 1)) {
...
func TestNewMatrix(t *testing.T) {
	...
	*/
	matrix := geometry.NewMatrix(4, 4, [][]float64{{1, 2, 3, 4}, {5.5, 6.5, 7.5, 8.5}, {9, 10, 11, 12}, {13.5, 14.5, 15.5, 16.5}})
...
func Test2x2Matrix(t *testing.T) {
	matrix := geometry.NewMatrix(2, 2, [][]float64{{-3, 5}, {1, -2}})
...
func Test3x3Matrix(t *testing.T) {
	matrix := geometry.NewMatrix(3, 3, [][]float64{{-3, 5, 0}, {1, -2, -7}, {0, 1, 1}})
...
func TestMatrixEquality(t *testing.T) {
	m1 := geometry.NewMatrix(4, 4, [][]float64{{1, 5, 9, 5}, {2, 6, 8, 4}, {3, 7, 7, 3}, {4, 8, 6, 2}})
	m2 := geometry.NewMatrix(4, 4, [][]float64{{1, 5, 9, 5}, {2, 6, 8, 4}, {3, 7, 7, 3}, {4, 8, 6, 2}})
	if !m1.IsEqual(*m2) {
	...
	m3 := geometry.NewMatrix(4, 4, [][]float64{{1, 5, 9, 5}, {2, 6, 8, 4}, {3, 7, 7, 3}, {4, 8, 6, 2}})
	m4 := geometry.NewMatrix(4, 4, [][]float64{{2, 6, 8, 4}, {3, 7, 7, 3}, {1, 5, 9, 5}, {4, 8, 6, 2}})
	if m3.IsEqual(*m4) {
...
func TestMatrixMultiply_InvalidDimensions(t *testing.T) {
	m1 := geometry.NewMatrix(2, 3, [][]float64{
		{1, 2, 3},
		{4, 5, 6},
	})

	m2 := geometry.NewMatrix(2, 2, [][]float64{
		{7, 8},
		{9, 10},
	})
...
func TestMatrixMultiply(t *testing.T) {
	m1 := geometry.NewMatrix(4, 4, [][]float64{
		{1, 2, 3, 4},
		{5, 6, 7, 8},
		{9, 8, 7, 6},
		{5, 4, 3, 2},
	...
	m2 := geometry.NewMatrix(4, 4, [][]float64{
		{-2, 1, 2, 3},
		{3, 2, 1, -1},
		{4, 3, 6, 5},
		{1, 2, 7, 8},
	...
	expectedMatrix := geometry.NewMatrix(4, 4, [][]float64{
		{20, 22, 50, 48},
		{44, 54, 114, 108},
		{40, 58, 110, 102},
		{16, 26, 46, 42},
...
func TestMatrixMultiply_Tuple(t *testing.T) {
	...
	expectedMatrix := geometry.NewMatrix(4, 1, [][]float64{
		{18},
		{24},
		{33},
		{1},
...
func TestMatrixMultiply_IdentityMatrix(t *testing.T) {
	...
	result := m1.Multiply(*geometry.IdentityMatrix())
...
func TestMatrixMultiply_Transpose(t *testing.T) {
	...
	expected := geometry.NewMatrix(4, 4, [][]float64{
		{0, 9, 1, 0},
		{9, 8, 8, 0},
		{3, 0, 5, 5},
		{0, 8, 3, 8},
...
func TestMatrixMultiply_TransposeIdentity(t *testing.T) {
	m1 := geometry.IdentityMatrix()

	result := m1.Transpose()

	if !result.IsEqual(*geometry.IdentityMatrix()) {
		t.Errorf("Expected %v, but got %v", geometry.IdentityMatrix().Value, result.Value)
	}
}

func TestMatrix_2x2Determinant(t *testing.T) {
	m := geometry.NewMatrix(2, 2, [][]float64{
		{1, 5},
		{-3, 2},
	})
	result := geometry.Determinant2(*m)
	expected := 17.0
...
func TestMatrix_3x3SubMatrix(t *testing.T) {
	m1 := geometry.NewMatrix(3, 3, [][]float64{
		{1, 5, 0},
		{-3, 2, 7},
		{0, 6, -3},
	})

	expected := geometry.NewMatrix(2, 2, [][]float64{
		{-3, 2},
		{0, 6},
	})
...
func TestMatrix_4x4SubMatrix(t *testing.T) {
	...
	expected := geometry.NewMatrix(3, 3, [][]float64{
		{1, 3, 4},
		{2, -4, 2},
		{0, 11, 1},
	})
...
func TestMatrix_3x3Minor(t *testing.T) {
	m := geometry.NewMatrix(3, 3, [][]float64{
		{3, 5, 0},
		{2, -1, -7},
		{6, -1, 5},
	})
	result := geometry.Minor3(*m, 1, 0)
	expected := 25.0
	if !util.IsFloatEqual(result, expected) {
		t.Errorf("Expected %v, but got %v", expected, result)
	}
...
func TestMatrix_3x3Cofactor(t *testing.T) {
	...
	result = geometry.Cofactor3(*m, 0, 0)
	expected = -12.0
	if !util.IsFloatEqual(result, expected) {
		t.Errorf("Expected %v, but got %v", expected, result)
	}

	result = geometry.Cofactor3(*m, 1, 0)
	expected = -25.0
	if !util.IsFloatEqual(result, expected) {
		t.Errorf("Expected %v, but got %v", expected, result)
	}
...
func TestMatrix_3x3Determinant(t *testing.T) {
	...
	matrix := geometry.NewMatrix(3, 3, [][]float64{
		{1, 2, 6},
		{-5, 8, -4},
		{2, 6, 4},
	})
	...
	// Test cofactor(A, 0, 0)
	result := geometry.Cofactor3(*matrix, 0, 0)
	expected := 56.0
	if !util.IsFloatEqual(result, expected) {
	...
	// Test cofactor(A, 0, 1)
	result = geometry.Cofactor3(*matrix, 0, 1)
	expected = 12.0
	if !util.IsFloatEqual(result, expected) {
	...
	// Test cofactor(A, 0, 2)
	result = geometry.Cofactor3(*matrix, 0, 2)
	expected = -46.0
	if !util.IsFloatEqual(result, expected) {
	...
	// Test determinant(A)
	result = geometry.Determinant3(*matrix)
	expected = -196.0
	if !util.IsFloatEqual(result, expected) {
...
func TestMatrix_4x4Determinant(t *testing.T) {
	...
	matrix := geometry.NewMatrix(4, 4, [][]float64{
		{-2, -8, 3, 5},
		{-3, 1, 7, 3},
		{1, 2, -9, 6},
		{-6, 7, 7, -9},
	...
	result := geometry.Cofactor4(*matrix, 0, 0)
	expected := 690.0
	if !util.IsFloatEqual(result, expected) {
		t.Errorf("Expected cofactor(A, 0, 0) = %v, but got %v", expected, result)
	}
	...
	// Test cofactor(A, 0, 1)
	result = geometry.Cofactor4(*matrix, 0, 1)
	expected = 447.0
	if !util.IsFloatEqual(result, expected) {
		t.Errorf("Expected cofactor(A, 0, 1) = %v, but got %v", expected, result)
	}
	...
	// Test cofactor(A, 0, 2)
	result = geometry.Cofactor4(*matrix, 0, 2)
	expected = 210.0
	if !util.IsFloatEqual(result, expected) {
		t.Errorf("Expected cofactor(A, 0, 2) = %v, but got %v", expected, result)
	}
	...
	// Test cofactor(A, 0, 3)
	result = geometry.Cofactor4(*matrix, 0, 3)
	expected = 51.0
	if !util.IsFloatEqual(result, expected) {
...
func TestMatrix_InvertibleMatrix(t *testing.T) {
	...
	// Det is 0
	matrix2 := geometry.NewMatrix(4, 4, [][]float64{
		{-4, 2, -2, -3},
		{9, 6, 2, 6},
		{0, -5, 1, -5},
		{0, 0, 0, 0},
...
func TestMatrix_Inverse(t *testing.T) {
	...
	// Given the following 4x4 matrix A
	matrix1 := geometry.NewMatrix(4, 4, [][]float64{
		{9, 3, 0, 9},
		{-5, -2, -6, -3},
		{-4, 9, 6, 4},
		{-7, 6, 6, 2},
	...
	// Expected inverse(A)
	expected1 := geometry.NewMatrix(4, 4, [][]float64{
...
func TestMatrix_ProductWithInverse(t *testing.T) {
	...
	// Given the following 4x4 matrix A
	matrixA := geometry.NewMatrix(4, 4, [][]float64{
		{3, -9, 7, 3},
		{3, -8, 2, -9},
		{-4, 4, 4, 1},
		{-6, 5, -1, 1},
	...
	matrixB := geometry.NewMatrix(4, 4, [][]float64{
		{8, 2, 2, 2},
		{3, -1, 7, 0},
		{7, 0, 5, 4},
		{6, -2, 0, 5},
...
func TestTranslatePoint(t *testing.T) {
	p := geometry.NewPoint(-3, 4, 5)
	result := p.Translate(5, -3, 2)
	if !result.IsEqual(*geometry.NewPoint(2, 1, 7)) {
		t.Errorf("got: %+v, want: (2, 1, 7)", result)
	}

	p1 := geometry.NewPoint(-3, 4, 5)
	tM := geometry.TranslationM(5, -3, 2).Inverse()
	result1 := tM.Multiply(*p1.ToMatrix()).ToPoint()
	if !result1.IsEqual(*geometry.NewPoint(-8, 7, 3)) {
...
func TestScalingMatrixAppliedToVector(t *testing.T) {
	...
	v := geometry.NewVector(-4, 6, 8)

	// When scaling is applied with factors (2, 3, 4)
	result := v.Scale(2, 3, 4)

	// Then the result should be vector(-8, 18, 32)
	expected := geometry.NewVector(-8, 18, 32)
...
func TestMultiplyingByInverseOfScalingMatrix(t *testing.T) {
	...
	transform := geometry.ScaleM(2, 3, 4)
	...
	expected := geometry.NewVector(-2, 2, 2)
...
func TestReflectionIsScalingByNegativeValue(t *testing.T) {
	...
	expected := geometry.NewPoint(-2, 3, 4)
...
func TestRotatingPointAroundXAxis(t *testing.T) {
	// Given p ← point(0, 1, 0)
	p := geometry.NewPoint(0, 1, 0)

	// And half_quarter ← rotation_x(π / 4)
	halfQuarter := geometry.RotateXM(math.Pi / 4)

	// And full_quarter ← rotation_x(π / 2)
	fullQuarter := geometry.RotateXM(math.Pi / 2)

	// Then half_quarter * p = point(0, √2/2, √2/2)
	resultHalfQuarter := halfQuarter.Multiply(*p.ToMatrix()).ToPoint()
	expectedHalfQuarter := geometry.NewPoint(0, math.Sqrt(2)/2, math.Sqrt(2)/2)
	...
	expectedFullQuarter := geometry.NewPoint(0, 0, 1)
...
func TestInverseOfXRotationRotatesOppositeDirection(t *testing.T) {
	...
	expected := geometry.NewPoint(0, math.Sqrt(2)/2, -math.Sqrt(2)/2)
...
func TestRotatingPointAroundYAxis(t *testing.T) {
	// Given p ← point(0, 0, 1)
	p := geometry.NewPoint(0, 0, 1)

	// And half_quarter ← rotation_y(π / 4)
	halfQuarter := geometry.RotateYM(math.Pi / 4)

	// And full_quarter ← rotation_y(π / 2)
	fullQuarter := geometry.RotateYM(math.Pi / 2)

	// Then half_quarter * p = point(√2/2, 0, √2/2)
	resultHalfQuarter := halfQuarter.Multiply(*p.ToMatrix()).ToPoint()
	expectedHalfQuarter := geometry.NewPoint(math.Sqrt(2)/2, 0, math.Sqrt(2)/2)
	...
	resultFullQuarter := fullQuarter.Multiply(*p.ToMatrix()).ToPoint()
	expectedFullQuarter := geometry.NewPoint(1, 0, 0)
...
func TestRotatingPointAroundZAxis(t *testing.T) {
	...
	halfQuarter := geometry.RotateZM(math.Pi / 4)

	// And full_quarter ← rotation_z(π / 2)
	fullQuarter := geometry.RotateZM(math.Pi / 2)

	// Then half_quarter * p = point(-√2/2, √2/2, 0)
	resultHalfQuarter := halfQuarter.Multiply(*p.ToMatrix()).ToPoint()
	expectedHalfQuarter := geometry.NewPoint(-math.Sqrt(2)/2, math.Sqrt(2)/2, 0)
...
func TestShearingTransformation(t *testing.T) {
	...
	expected := geometry.NewPoint(6, 3, 4)
	...
	expected = geometry.NewPoint(2, 5, 4)
	...
	expected = geometry.NewPoint(2, 7, 4)
	...
	expected = geometry.NewPoint(2, 3, 6)
	...
	expected = geometry.NewPoint(2, 3, 7)
...
func TestChainingTransformations(t *testing.T) {
	...
	p := geometry.NewPoint(1, 0, 1)
	pM := p.ToMatrix()
	rotateXM := geometry.RotateXM(math.Pi / 2)
	scaleM := geometry.ScaleM(5, 5, 5)
	translationM := geometry.TranslationM(10, 5, 7)

	// apply rotation first
	pRotateM := rotateXM.Multiply(*pM)
	pRotate := pRotateM.ToPoint()
	expected := geometry.NewPoint(1, -1, 0)
	...
	expected = geometry.NewPoint(5, -5, 0)
	...
	expected = geometry.NewPoint(15, 0, 7)
	...
	chainedTransformM := geometry.ChainTransforms([]*geometry.Matrix{
		pM,
		geometry.RotateXM(math.Pi / 2),
		geometry.ScaleM(5, 5, 5),
		geometry.TranslationM(10, 5, 7),
	})
